...
class SendAttendanceReportToHodJob implements ShouldQueue
{
    ...
    public function __construct(
        ...
        public array $additionalToEmails = [],
    ...
    public function handle(): void
    {
        ...
        $allToEmails = array_values(array_unique(array_filter(array_merge(
            [$this->hodEmail],
            $this->additionalToEmails
        ))));
        $allCcEmails = array_values(array_unique(array_filter($this->ccEmails)));

        Log::channel('single')->info('SendAttendanceReportToHodJob recipients', [
            'to' => $allToEmails,
            'cc' => $allCcEmails,
        ]);
        ...
        ], function ($message) use ($reportData, $pdf, $fileName, $allToEmails, $allCcEmails) {

            $message->to($allToEmails)
